...
bool load_filament(const float &slow_load_length/*=0*/, const float &fast_load_length/*=0*/, const float &purge_length/*=0*/, const int8_t max_beep_count/*=0*/,
  ...
  if (pause_for_user) {
    ...
    KEEPALIVE_STATE(PAUSED_FOR_USER);
      ...
      const char tool = '0'
        ...
        #endif
      ;
      host_action_prompt_begin(PROMPT_USER_CONTINUE, GET_TEXT(MSG_FILAMENTLOAD));
  ...
  if (fast_load_length) {
    ...
    TERN_(HOST_PROMPT_SUPPORT, host_prompt_do(PROMPT_USER_CONTINUE, PSTR("Filaman Tasfiye Ediliyor"), DISMISS_STR));
...
void wait_for_confirmation(const bool is_reload/*=false*/, const int8_t max_beep_count/*=0*/ DXC_ARGS) {
  ...
  while (wait_for_user) {
    ...
    if (nozzle_timed_out) {
      ...
      TERN_(HOST_PROMPT_SUPPORT, host_prompt_do(PROMPT_INFO, GET_TEXT(MSG_REHEATING), CONTINUE_STR));
      ...
      HOTEND_LOOP() thermalManager.heater_idle[e].start(nozzle_timeout);
      TERN_(HOST_PROMPT_SUPPORT, host_prompt_do(PROMPT_USER_CONTINUE, PSTR("Isınma Tamam"), CONTINUE_STR));
